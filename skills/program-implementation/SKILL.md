...
требование, нарушающее принципы дисциплины (большой модуль, два I/O
в одном slice'е, отсутствие антецедента, рассогласование контрактов
с графом) — **остановиться и сообщить**. Не реализовывать «как-нибудь».
...
1. Все модули реализованы и юнит-тесты зелёные → запустить компонентный
   сценарий happy path — должен стать зелёным.
2. Реализован обработчик режима отказа → запустить компонентный сценарий
   соответствующего отказа — должен стать зелёным.
3. К концу Шага 3 **все компонентные сценарии этого slice'а зелёные**.

Если компонентный сценарий зелёным не становится, а юнит-тесты модулей
зелёные — **проблема не в коде, а в спецификации**: либо контракт slice'а
не соответствует тому, что обещано в OpenAPI/AsyncAPI, либо
`contracts-graph.md` не учёл какой-то путь. Стоп, сообщить оператору,
opus возвращается на Шаг 9 проверить согласованность.
...
«1 happy + по тесту на каждую ветку антецедента». Головной модуль
slice'а покрывается юнит-тестом отдельно (один happy + ветки разбора
ошибок от конструкторов и I/O). Ингресс-адаптер юнит-тестами не покрывается
— его проверяют компонентные тесты slice'а через его реальный вход
(HTTP-запрос, публикация в брокер, gRPC-вызов или CLI-исполнение).
Инфраструктурный модуль тоже без юнитов — только компонентные тесты.

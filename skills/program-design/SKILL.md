...
Для каждого модуля **логики** (конструкторы доменных структур, чистые
функции над ними, головной модуль слайса):

```
N_юнит_тестов = 1 (happy path) + Σ (ветки антецедента)
```

**Жёсткое правило: I/O-модули и ингресс-адаптер юнитами не покрываются.**

I/O-модули по сути **трубы** — переносят байты между процессом и внешней
зависимостью (БД, брокер, внешний API, файловая система). Бизнес-логики
в них нет, тестировать нечего. «Юнит-тест» против `:memory:` БД или
in-memory broker — это маленький интеграционный тест, а не юнит.
Драйвер реальный, обёртка реальная, in-memory — всего лишь техническая
деталь хранения.

Ингресс-адаптер по той же причине: парсит вход, маппит ошибки в формат
ответа — никакого «алгоритма», который надо проверять юнитом.
...
| Головной модуль слайса            | да, happy + ветки разбора ошибок          | да, happy path + Then-шаги                                              |
...
- [ ] **В таблице юнит-тестов каждой карточки слайса нет I/O-модулей и нет ингресс-адаптера: I/O — трубы, проверяются только компонентными сценариями (Шаг 8.1)**

...
func (h *httpHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusBadRequest, "VALIDATION_ERROR", "cannot read request body")
		return
	}

	req := RegistrationFinishRequest{
		RegistrationIDRaw: rawID,
		AttestationBody:   body,
	}

	resp, err := ProcessRegistrationFinish(req, h.deps)
	if err != nil {
		h.mapError(w, err)
		return
	}
...
func (h *httpHandler) mapError(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrInvalidRegID),
		errors.Is(err, ErrAttestationParse):
		writeError(w, http.StatusUnprocessableEntity, "VALIDATION_ERROR", err.Error())
	case errors.Is(err, ErrSessionNotFound),
		errors.Is(err, ErrSessionExpired):
		writeError(w, http.StatusNotFound, "NOT_FOUND", err.Error())
	case errors.Is(err, ErrAttestationInvalid):
		writeError(w, http.StatusUnprocessableEntity, "ATTESTATION_INVALID", err.Error())
	case errors.Is(err, ErrHandleTaken):
		writeError(w, http.StatusUnprocessableEntity, "HANDLE_TAKEN", err.Error())

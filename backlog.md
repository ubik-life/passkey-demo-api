...
### Шаг 5 — Ретро по архитектурным развилкам S5 (после реализации S6)

После того, как все 6 эндпоинт-слайсов реализованы и смержены в main (S1-S6), пересмотреть сознательно сабоптимальные выборы, сделанные при дизайне S5 (PR #32). Источник обоснований — `docs/design/passkey-demo/slices/05-sessions-logout.md` секция «Решения по дизайну».

**Развилка 1. Семантика logout: «текущая сессия» vs «все сессии».**

S5 реализован как **(A) logout-from-all**: `UPDATE refresh_tokens SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL`. Минимальный путь, без миграций, демо-уровень. OpenAPI говорит «инвалидирует refresh token текущей сессии» (singular), но в текущем JWT нет связи access ↔ конкретный refresh.

Отложенные альтернативы:
- **(B)** Связать access ↔ refresh через `jti`-claim: новое поле в JWT, новая колонка `refresh_tokens.access_jti`, миграция, возврат к S2 за изменением `generateTokenPair`. Точнее, multi-device-aware logout.
- **(C)** Ввести таблицу `sessions` и `session_id` в JWT. Полное переосмысление модели сессий.

**Когда пересматривать:** если появляется UI с multi-device-флоу или сценарий, где «выйти со всех устройств» становится UX-проблемой.

**Развилка 2. Где живёт `VerifyAccessToken`.**

S5 реализован как **(α) аддитивный экспорт из S2** (`registrations_finish.VerifyAccessToken`) — по паттерну `GenerateTokenPair`/`BuildResponse`. S6 импортирует из того же места.

Отложенная альтернатива:
- **(β)** Извлечь в отдельный пакет `internal/auth/`. Меньше связности между слайсами, но ломает паттерн «слайс владеет своими экспортами» (прецедентов в проекте нет).

**Когда пересматривать:** после реализации S6 — станет видно, остаётся ли `VerifyAccessToken` чисто S2-владением или фактически стал общей auth-инфраструктурой. Сигнал к переезду — рост auth-связанного кода в S2 настолько, что слайс-пакет становится перегруженным.

**Тикеты:**

- [ ] **T5.1 — Ретро (A) vs (B) vs (C).** После мержа S6 проверить: появились ли реальные пользователи с multi-device, поступили ли жалобы на «выйти со всех устройств». Если нет — пометить (A) как final, тикет закрыть. Если да — спроектировать (B) opus'ом на скилле `program-design`.
- [ ] **T5.2 — Ретро (α) vs (β).** После мержа S6 посчитать: сколько строк в `internal/slice/registrations_finish/` связано с auth (verify, sign, claims) vs с регистрацией. Если auth-часть > 30% и есть третий потребитель — спроектировать переезд в `internal/auth/`.

**Зачем «после S6», не «после S5».** На S5 единственный потребитель `VerifyAccessToken` — сам S5. Мерять перегруженность S2 имеет смысл только когда у `VerifyAccessToken` появляется второй честный потребитель (S6), и можно сравнить «два пакета, одна функция» против «один общий пакет».

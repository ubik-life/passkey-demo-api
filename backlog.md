...
**Bootstrap: placeholder-сервис.** Шаг 2.0 нужно закрыть одним PR, но сервис из Шага 3 ещё не написан, и smoke-сценарию (T2.0.12) не во что стрелять. Решение: в `compose/service.Dockerfile` собирается **временный заглушечный `main.go`** — HTTP-сервер, который биндится на порт и отдаёт `401 Unauthorized` на всё. Он живёт в `cmd/api/main.go` и постепенно вытесняется реальной логикой в Шаге 3 (TDD-цикл первый модуль за модулем заменит хендлеры). Это сохраняет принцип «контракт и тесты первичны»: тесты красные не потому, что нет сервера, а потому, что сервер не реализует контракт. Альтернативы (отложить smoke, переставить шаги) либо ломают атомарность Шага 2.0, либо нарушают флоу AGENTS.md §9.
...
- [ ] **T2.0.3 — `compose/service.Dockerfile` + placeholder `cmd/api/main.go`.** Multi-stage build: создать минимальный `cmd/api/main.go` (отдаёт `401 Unauthorized` на любой запрос, биндится на `:8080`) и Dockerfile, собирающий его. Это бутстрап, чтобы T2.0.12 smoke-тест мог стрелять в живой сервис. В Шаге 3 хендлеры этого `main.go` будут постепенно вытеснены реальными модулями.

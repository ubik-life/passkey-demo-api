...
|---|---|---|---|
| SQLite | sync | Недоступна | `503 Service Unavailable`, `error.code=db_unavailable` |

Сервис не различает типы отказов БД (таймаут, недоступность, нарушение целостности) — все они мапятся в одну ошибку `db_unavailable`. Это проектное решение: одна различимая ветка в адаптере → один компонентный сценарий отказа.

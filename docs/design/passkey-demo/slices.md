...
|---|-----------|---------------|-------------|------------------|--------|
| 1 | HTTP | `POST /v1/registrations` | `registrations-start` | Фаза 1 регистрации: принять handle, создать challenge + регистрационную сессию, вернуть `{id, options}` | спроектирован |
| 2 | HTTP | `POST /v1/registrations/{id}/attestation` | `registrations-finish` | Фаза 2 регистрации: верифицировать attestation, создать пользователя и credential, выдать JWT-пару | todo |

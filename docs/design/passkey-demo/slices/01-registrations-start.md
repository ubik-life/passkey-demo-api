...
- Покрытие 100% строк/веток модулей логики достигается этими 11 юнит-тестами.
- Головной модуль, I/O-модуль и ингресс-адаптер не входят в метрику покрытия юнитов; их корректность доказана компонентными сценариями (черный ящик через реальный HTTP).

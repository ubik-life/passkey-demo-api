...
S1 спроектирован и реализован (PR #17). S2 спроектирован и реализован (PR #21). S3 спроектирован и реализован (PR #26). Техдолг S1/S2 → Store-объект закрыт (PR #27). S4 спроектирован (PR #28) и реализован (PR #30). S5 спроектирован — текущая итерация, ожидает аппрува оператора и реализации sonnet'ом. S6 проектируется отдельной итерацией opus'а после S5.

---

## Хендофф-чеклист S5 (заполняет opus, проверяет оператор)
...
- [ ] Оператор аппрувит пакет S5 — @maxmorev, <YYYY-MM-DD>
...
## Следующие итерации (S6)

S5 спроектирован — текущая итерация opus'а (ветка `feat/design-sessions-logout`), ожидает аппрува оператора и реализации sonnet'ом. После реализации S5 — следующая итерация S6. Каждый слайс — отдельная ветка `feat/design-<slice>`, отдельный PR, отдельный хендофф-чеклист, подписанный оператором.

Очерёдность:

- **S6** `slices/06-users-me.md` — `GET /v1/users/me` (профиль пользователя по access token) — **следующая итерация после S5**

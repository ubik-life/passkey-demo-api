...
S1 спроектирован и реализован (PR #17). S2 спроектирован и реализован (PR #21). S3 спроектирован и реализован (PR #26). Техдолг S1/S2 → Store-объект закрыт (PR #27). S4 спроектирован (PR #28) и реализован (PR #30). S5–S6 проектируются отдельными итерациями opus'а — следующая итерация: дизайн S5 (`sessions-logout`).
...
S4 реализован (PR #30) и смержен в main. Следующая итерация — дизайн S5 opus'ом на скилле `program-design`. Каждый слайс — отдельная ветка `feat/design-<slice>`, отдельный PR, отдельный хендофф-чеклист, подписанный оператором.

Очерёдность:

- **S5** `slices/05-sessions-logout.md` — `DELETE /v1/sessions/current` (отзыв refresh token) — **следующая итерация (дизайн)**

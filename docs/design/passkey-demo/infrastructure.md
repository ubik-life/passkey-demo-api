...
## Подключение слайса 6 (S6)

```go
// internal/slice/users_me/register.go
package users_me

func Register(mux chi.Router, deps Deps) {
    h := newHTTPHandler(deps)
    mux.Get("/v1/users/me", h.ServeHTTP)
}

// Deps — зависимости слайса 6.
// Применение Шага 6 скилла + feedback_io_autonomous_store: БД-зависимость
// инкапсулирована в *Store, головной модуль её не видит. Сырого *sql.DB здесь нет.
type Deps struct {
    Store    *Store                          // автономный I/O-объект слайса 6, инкапсулирующий *sql.DB
    Clock    clock.Clock
    Logger   *slog.Logger
    Verifier ed25519.PublicKey               // парный публичный ключ к Signer.Private (тот же Signer-структуры из wire.go; второе использование, первое — в S5)
    JWT      registrations_finish.JWTConfig  // импорт из S2; используется поле Issuer для проверки claim
}
```
...
```go
// internal/app/wire.go (фрагмент для S6)
usersMeStore := users_me.NewStore(db)  // db *sql.DB — общий пул из db.Open
deps.UsersMe = users_me.Deps{
    Store:    usersMeStore,
    Clock:    clk,
    Logger:   log.With("slice", "users-me"),
    Verifier: signer.Public,             // тот же публичный ключ, что у S5
    JWT:      cfg.JWT,
}
```

В S6 не нужны `RP`, `Signer.Private`, `ChallengeTTL`: read-эндпоинт не верифицирует assertion (нет WebAuthn-операции) и не подписывает новые токены. По набору `Deps` S6 идентичен S5 (оба — Bearer-auth read/write эндпоинты с минимумом зависимостей).

**Поле `Verifier ed25519.PublicKey` — второе использование в проекте.** Первое — в S5 (`infrastructure.md` секция выше). Никаких изменений в `generateSigner()` не требуется: в `wire.go` S6 берёт ровно тот же `signer.Public`.

**Никаких новых миграций S6 не вводит.** Слайс читает таблицу `users` (S2 миграция `0002`), один SELECT по PK. Никаких ALTER TABLE / новых индексов / новых файлов в `internal/db/migrations/` для S6 не создавать. PK на `id` (TEXT PRIMARY KEY) уже даёт O(log n) lookup; индекс по `handle` (S2 `0002_users.sql`) на этом эндпоинте не используется.
...
### S6 — без новых миграций

S6 (`users-me`) использует существующую таблицу `users` (S2 миграция `0002`). Один `SELECT id, handle, created_at FROM users WHERE id = ?` — выполняется без изменения схемы.

Дополнительный индекс не нужен: PK на `id` (TEXT PRIMARY KEY) уже даёт O(log n) lookup. OpenAPI-схема `User { id, handle }` собирается из существующих колонок без расширения таблицы.

### Будущие миграции

Возможны при дальнейшем расширении профиля пользователя (email, avatar URL, флаги фич) — потребуют отдельных слайсов и миграций ALTER TABLE. На текущей дорожке (S1–S6) не запланированы.
